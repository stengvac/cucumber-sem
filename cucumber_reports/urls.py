...
urlpatterns = [
    ...
    #Statistics
    url(r'statistics/img/(?P<name>[\w|\W]+)/temp.png$', views.render_img, name='img'),
    url(r'statistics/build/(?P<name>[\w|\W]+)/(?P<number>[\w|\W]+)/$', views.BuildRunStatisticsView.as_view(), name='statistics_build'),
    url(r'statistics/overall/(?P<name>[\w|\W]+)$', views.StatisticBuildOverTimeView.as_view(), name='statistics_overall'),
